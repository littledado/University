...
#define MAX_CHAR 31
#define FILE_IN "C:\\Users\\Davide Trenti\\Desktop\\Progetti\\University\\AlgoritmiEProgrammazione\\Laboratori\\Lab02\\Esercizio2\\corse.txt"
...
typedef struct
{
    int n_record;
    trip *trips;
    trip **tripsReference_DATE;
    trip **tripsReference_CODE;
    trip **tripsReference_DEPARTURE;
    trip **tripsReference_ARRIVAL;
    SortingType sorted;
} trip_table;
...
void readTableFromFile(FILE *fp, trip_table *tableRef);
...
void readTableFromFile(FILE *fp, trip_table *tableRef)
{
    ...
    //Check empty trip table
    if (isEmptyTripTable(tableRef) == 0)
    {
        freeTripTable(tableRef);
    }

    fscanf(fp, "%d", &arrayDimension);
    // Allocate memory for data struct
    initializeTripTable(tableRef, arrayDimension);

    for (int i = 0; i < tableRef->n_record; i++)
    {
        trip trip;
        ...
        // save record
        tableRef->trips[i] = trip;

        // Save pointers
        tableRef->tripsReference_DATE[i] = tableRef->tripsReference_CODE[i] = tableRef->tripsReference_DEPARTURE[i] = tableRef->tripsReference_ARRIVAL[i] = &tableRef->trips[i];
    }
    // Set no sorting
    tableRef->sorted = NONE;
}
...
comando_e readCommandFromUser()
{
    ...
    printf("Comandi disponibili: 'date', 'partenza', 'capolinea', 'ritardo', 'ritardo_tot', 'ordina', 'stampa', 'cerca', 'leggi'  e  'fine'\n"
           "Inserisci comando:\t");
    ...
    if (strcmp(commandString, "LEGGI") == 0)
        return r_read_data;
    return r_fine;
}
void executeCommand(comando_e comando, trip_table *table)
...
trip_table getEmptyTripTable()
{
    ...
    table.n_record = 0;
    table.trips = NULL;
    table.tripsReference_ARRIVAL = NULL;
    table.tripsReference_CODE = NULL;
    table.tripsReference_DATE = NULL;
    table.tripsReference_DEPARTURE = NULL;
    table.sorted = NONE;
...
int isEmptyTripTable(trip_table *table)
{
    return (table->n_record == 0) ? 1 : 0;
}
void freeArray2D(void ***pointer, int rows, int columns)
{
...
void initializeTripTable(trip_table *table, int n_records)
{
    table->n_record = n_records;
    mallocArray1D(&table->trips, table->n_record);
    mallocArray2DRef(&table->tripsReference_ARRIVAL, table->n_record);
    mallocArray2DRef(&table->tripsReference_CODE, table->n_record);
    mallocArray2DRef(&table->tripsReference_DATE, table->n_record);
    mallocArray2DRef(&table->tripsReference_DEPARTURE, table->n_record);
}
void readTableDataStruct(trip_table *table)
{
    char nameFile[MAX_CHAR];
    FILE *fp_in = NULL;
...
#endif
    // Open file
    openFile(&fp_in, nameFile);
    // Read data struct
    readTableFromFile(fp_in, table);

    printf("Base dati aggiornata\n");
    // Close file
    closeFile(&fp_in);
}